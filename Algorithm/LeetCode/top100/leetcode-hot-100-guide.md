...
**探路者**（快指针 `f`）从左往右翻看每一格：遇到一本书（非零）？递给身后的**整理者**（慢指针 `s`），整理者把书放在下一个空位上，然后 `s` 前进一步。遇到空格（零）？跳过，只有 `f` 前进。

探路者走得快，整理者走得慢。等探路者走完整排书架，整理者后面全是空位。两个人朝同一个方向走，但速度不同——快的负责发现，慢的负责安置。

```
以 LeetCode 283「移动零」为例: 把0移到末尾，保持非零元素顺序
...
第2步: f 看到 1（非零!），把 1 放到 s 的位置，s 前进
       [1,  1,  0,  3,  12]
            s
            f
...
第4步: f 看到 3（非零!），放到 s 的位置，s 前进
       [1,  3,  0,  3,  12]
                s
                    f

第5步: f 看到 12（非零!），放到 s 的位置，s 前进
       [1,  3,  12, 3,  12]
                    s
                        f

f 走完了，把 s 之后的位置全部填 0:
       [1,  3,  12, 0,  0]
                    s           完成!
```

关键在于：`s` 左边（不含 `s`）都是已整理好的非零元素，`s` 和 `f` 之间都是可以被覆盖的区域。**两个指针同向前进，快的"读"，慢的"写"**——这就是同向指针的本质。
